...
if avgeverynth==3
	...
	for i=1:length(UniqueEdges)
	  ...
	  for j=b
	    ...
	    if k==1
	    	Ktrailing=FeP2P3S.K1[Idx_trailing]
	    elseif k==2
	    	Ktrailing=FeP1P3S.K1[Idx_trailing]
	    elseif k==3
	    	Ktrailing=FeP1P2S.K1[Idx_trailing]
	    end    	
	    (k,Idx_future) =GrabPointNew6(InnerPoints,P1,P2,P3,j_future)
		if k==1
	    	Kfuture=FeP2P3S.K1[Idx_future]
	    elseif k==2
	    	Kfuture=FeP1P3S.K1[Idx_future]
	    elseif k==3
	    	Kfuture=FeP1P2S.K1[Idx_future]
	    end   
	    (k,Idx_current) =GrabPointNew6(InnerPoints,P1,P2,P3,j_current)
		if k==1
			avg=(Ktrailing+FeP2P3S.K1[Idx_current]+Kfuture)/3;
			if isnan(avg)
				continue
			end
			FeP2P3S.K1[Idx_current]=avg
	    elseif k==2
	    	avg=(Ktrailing+FeP1P3S.K1[Idx_current]+Kfuture)/3
			if isnan(avg)
				continue
			end
	    	FeP1P3S.K1[Idx_current]=(Ktrailing+FeP1P3S.K1[Idx_current]+Kfuture)/3
	    elseif k==3
	    	avg=(Ktrailing+FeP1P2S.K1[Idx_current]+Kfuture)/3    	
	    	if isnan(avg)
				continue
			end
	    	FeP1P2S.K1[Idx_current]=(Ktrailing+FeP1P2S.K1[Idx_current]+Kfuture)/3    	
	    end       
...
if avgeverynth==5
	...
	for i=1:length(UniqueEdges)
	  ...
	  for j=b
	    ...
	    if k==1
	    	Ktrailing2=FeP2P3S.K1[Idx_trailing]
	    elseif k==2
	    	Ktrailing2=FeP1P3S.K1[Idx_trailing]
	    elseif k==3
	    	Ktrailing2=FeP1P2S.K1[Idx_trailing]
	    end   
	    (k,Idx_future) =GrabPointNew6(InnerPoints,P1,P2,P3,j_future)
		if k==1
	    	Kfuture=FeP2P3S.K1[Idx_future]
		...
		if k==1
	    	Kfuture2=FeP2P3S.K1[Idx_future]
	    elseif k==2
	    	Kfuture2=FeP1P3S.K1[Idx_future]
	    elseif k==3
	    	Kfuture2=FeP1P2S.K1[Idx_future]
	    end   	    
	    (k,Idx_current) =GrabPointNew6(InnerPoints,P1,P2,P3,j_current)
		if k==1
			avg=(Ktrailing2+Ktrailing+FeP2P3S.K1[Idx_current]+Kfuture+Kfuture2)/5;
			if isnan(avg)
				continue
			end
			FeP2P3S.K1[Idx_current]=avg
	    elseif k==2
	    	avg=(Ktrailing2+Ktrailing+FeP1P3S.K1[Idx_current]+Kfuture+Kfuture2)/5
			if isnan(avg)
				continue
			end
	    	FeP1P3S.K1[Idx_current]=avg
	    elseif k==3
	    	avg=(Ktrailing2+Ktrailing+FeP1P2S.K1[Idx_current]+Kfuture+Kfuture2)/5   	
	    	if isnan(avg)
				continue
			end
	    	FeP1P2S.K1[Idx_current]=avg
	    end       
