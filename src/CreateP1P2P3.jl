function CreateP1P2P3( Triangles,Points )
...
P1=P1P2P3[1:end,1:3];        #extracts the variables P1-P3 into a array
P2=P1P2P3[1:end,4:6];
P3=P1P2P3[1:end,7:9];
...
#Internal function
function CreateP1P2P3_Int( P1P2P3,Triangles,Points,Val )
if Val==1 
	Indx=1:3;
elseif Val==2
	Indx=4:6;
...
for i = 1:length(Triangles[:,1]) #Assigns 'i' number of rows in triangles array
        P = (Triangles[i,Val]);  #Assigns P1 as all rows in the first column of triangles
        for j = 1:length(Points[:,1]) #Finds number of rows in imported table 'S'
            if Points[j,1] == P 
