function CalculateSSandDSDirs(CosAx,CosAy,CosAz)
# CalculateSSandDSDirs: Calculates the dipslip and strikeslip direction
...
# ( StrikeSlipCosine,DipSlipCosine ) = CalculateSSandDSDirs( cosd(45),0,cosd(45) )
#
#  Author: Tim Davis
...
Vstrike = cross(eZ,[CosAx[i];CosAy[i];CosAz[i]]);
    ...
    if norm(Vstrike)==0
        if CosAz[i,1]<1.0 #if the surface norm faces up
