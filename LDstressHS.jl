function LDstressHS(x,y,xe,ye,a,Beta,Ds,Dn,nu,E)
...
#FF8i =(YBi.*(one./((a + XBi).^2 + YBi.^2).^2 - one./(YBi.^2 + (a - XBi).^2).^2 + (two.*(a - XBi).*(two.*a - two.*XBi))./(YBi.^2 + (a - XBi).^2).^3 - (two.*(a + XBi).*(two.*a + two.*XBi))./((a + XBi).^2 + YBi.^2).^3))./(2*pi.*(nu - 1));
FF8i =(YBi.*(one./R2S2i - one./R1S2i + (two.*XMai.*(two.*a - two.*XBi))./R1S3i - (two.*XPai.*(two.*a + two.*XBi))./R2S3i))./(2*pi.*(nu - 1));
...
FF9i =(XMai./R1S2i + XPai./R2S2i)./(2*pi.*(nu - 1)) - (YBi.*((4*YBi.*XPai)./R2S3i + (4*YBi.*XMai)./R1S3i))./(2*pi.*(nu - 1));
